...
<a href="gestisci-responsabili.php" class="btn btn-outline-dark m-3 w-25 mb-5"><i class="fa-solid fa-circle-arrow-left me-2"></i>Torna indietro</a>
<div class="w-75 m-auto">
    <form action="modifica-responsabile.php" method="post">
        ...
        <div class="form-floating mb-3">
            <input type="text" class="form-control" name="username" placeholder="Nome" id="username" <?php
            if(isset($_GET["id"])){
                echo "value='". $username ."' readonly";
            }?> required>
            <label for="username">Username</label>
            <div class="invalid-feedback">Il responsabile deve avere uno username.</div>
        </div>
        <div class="form-floating mb-3">
    ...
    <button class="btn btn-danger" onclick="<?php if(isset($_GET["id"])) echo "eliminaResponsabile()"; else echo "location.href='gestisci-responsabili.php';";?>"><?php if(isset($_GET["id"])) echo "Elimina"; else echo "Annulla";?></button>
    <?php
    if (isset($_GET["id"]))
    {
    ?>
        <button type="button" class="btn btn-primary mx-auto" id="reset" onclick="resetPassword()">Reset password</button>
    <?php
    }
    ?>
...
<script>
    ...
    function resetPassword(){
        $.post("password-reset.php", {username: document.getElementById('username').value}, function(data){
            location.reload();
        });
    }
    <?php } ?>
